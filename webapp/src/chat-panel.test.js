...
describe('ChatPanel URL chip detection', () => {
  ...
  it('debounces rapid input — only one RPC call per pause', async () => {
    ...
    await settle(p);
    const ta = p.shadowRoot.querySelector('.input-textarea');
    // Fire three input events in quick succession.
    ...
    // Final text is what got dispatched.
  ...
  it('empty input clears detected chips', async () => {
    ...
    await settle(p);
    const ta = p.shadowRoot.querySelector('.input-textarea');
    // First, type something and wait for detection.
    ta.value = 'check https://a.com';
    ta.dispatchEvent(new Event('input'));
    await new Promise((r) => setTimeout(r, 350));
    await settle(p);
  ...
  it('stale detection responses are discarded', async () => {
    // Two calls in flight; the later one resolves first,
    // the earlier one resolves later with stale data.
    // The stale response must NOT update chips.
    let resolveFirst;
    let resolveSecond;
    const detect = vi.fn()
    ...
    await settle(p);
    const ta = p.shadowRoot.querySelector('.input-textarea');
    // First input event → debounce starts.
    ta.value = 'check https://stale.com';
    ta.dispatchEvent(new Event('input'));
    await new Promise((r) => setTimeout(r, 350));
    // First RPC call in flight.
    // Second input event → new debounce.
    ta.value = 'check https://fresh.com';
    ta.dispatchEvent(new Event('input'));
    await new Promise((r) => setTimeout(r, 350));
    // Now both are in flight. Resolve the SECOND one first.
    resolveSecond([
  ...
  it('detection failure is silent (no toast)', async () => {
    const detect = vi
      ...
      .mockRejectedValue(new Error('network down'));
    publishFakeRpc({ 'LLMService.detect_urls': detect });
    // Silence the expected debug log.
    const debugSpy = vi
      .spyOn(console, 'debug')
      .mockImplementation(() => {});
    const toastListener = vi.fn();
    ...
    try {
      ...
      await new Promise((r) => setTimeout(r, 350));
      await settle(p);
      // No toast — detection failure is best-effort.
      expect(toastListener).not.toHaveBeenCalled();
    } finally {
      window.removeEventListener('ac-toast', toastListener);
  ...
  it('does not run when RPC is disconnected', async () => {
    // No RPC published — rpcConnected is false.
    const p = mountPanel();
    await settle(p);
    const ta = p.shadowRoot.querySelector('.input-textarea');
    // Input events fire but _runUrlDetection bails early.
    ta.value = 'https://a.com';
    ta.dispatchEvent(new Event('input'));
    await new Promise((r) => setTimeout(r, 350));
    // No crash, no chips.
...
describe('ChatPanel URL chip fetch', () => {
  ...
  it('successful fetch transitions chip to fetched', async () => {
    ...
    // Click the fetch button on the detected chip.
    ...
    fetchBtn.click();
    await settle(panel);
    // Fetch RPC was called with the right args.
    expect(fetchUrl).toHaveBeenCalledWith(
      'https://a.com',
      true,
      true,
    );
    // Chip is now in fetched state.
  ...
  it('URLContent with error field produces errored chip (no toast)', async () => {
    // When the backend returns a URLContent payload with
    // an error (e.g., HTTP 404), the chip goes to errored
    // but we don't toast — the error is visible on the
    // chip itself.
    const { panel, fetchUrl } = await setupWithDetected();
    fetchUrl.mockResolvedValue({
      url: 'https://a.com',
    ...
    try {
      ...
      // No toast for payload-reported errors.
      expect(toastListener).not.toHaveBeenCalled();
    } finally {
      window.removeEventListener('ac-toast', toastListener);
    }
  ...
  it('chip transitions through fetching state before fetched', async () => {
    // Verify the intermediate state. We delay the RPC so
    // the fetching state is observable.
    let resolveFetch;
    const detect = vi
      .fn()
      .mockResolvedValue([
    ...
    fetchBtn.click();
    await settle(p);
    // Before RPC resolves, chip is in fetching state.
...
describe('ChatPanel URL chip remove', () => {
  it('calls remove_fetched_url and removes chip optimistically', async () => {
    ...
    await settle(p);
    // Click the remove button on the fetched chip.
  ...
  it('RPC failure does not restore the chip', async () => {
    // The optimistic remove stays — we don't try to undo
    // it if the server call fails. User clicked remove;
    // they want it gone.
    const remove = vi
      .fn()
      .mockRejectedValue(new Error('server down'));
...
describe('ChatPanel URL chip view dialog', () => {
  it('opens dialog with cached content on view event', async () => {
    ...
    await settle(p);
    // Click the fetched chip label to trigger view.
  ...
  it('falls back to get_url_content when chip content missing', async () => {
    ...
    await settle(p);
    const chipsEl = p.shadowRoot.querySelector('ac-url-chips');
    // Manually inject a fetched chip with NO content
    // (edge case — could happen if the chip was restored
    // from a future persistence layer without the body).
    chipsEl.updateDetected([
      { url: 'https://a.com', type: 'generic', display_name: 'a' },
    ]);
    ...
    label.click();
    await settle(p);
    // Fallback RPC was called.
    expect(getContent).toHaveBeenCalledWith('https://a.com');
    // Dialog rendered with fetched content.
  ...
  it('renders symbol map when present', async () => {
    ...
    await settle(p);
    const dialog = p.shadowRoot.querySelector('.url-view-dialog');
    // Symbol Map tab button is always visible in the tab bar
    // when a symbol map is present.
    expect(dialog.textContent).toContain('Symbol Map');
    // The dialog defaults to the Content tab per spec
    // ("active tab resets to `Content` each time the dialog
    // opens"), so the symbol map body isn't rendered yet.
    // Click the Symbol Map tab to reveal it.
  ...
  it('close button closes the dialog', async () => {
    ...
    chipsEl.shadowRoot
      ...
      .click();
    await settle(p);
    // The dialog has its own close button labelled "✕ Close".
    const buttons = p.shadowRoot.querySelectorAll(
  ...
  it('click inside dialog content does not close', async () => {
    ...
    chipsEl.shadowRoot
      ...
      .click();
    await settle(p);
    // Click on the body content itself.
...
describe('ChatPanel URL chip lifecycle', () => {
  it('session-changed clears all chips', async () => {
    ...
    // Session change — all chips cleared.
    pushEvent('session-changed', {
      session_id: 'sess_new',
      messages: [],
    });
  ...
  it('send clears detected/fetching but preserves fetched', async () => {
    ...
    await p._send();
    await settle(p);
    // Detected gone, fetched + errored survive.
