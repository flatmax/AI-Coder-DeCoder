...
**File selection not restored on server restart:** Auto-restore recovers conversation messages but does not restore file selection. After a server restart, the browser receives an empty `selected_files` list from `get_current_state()`. Users must re-select files after a server restart. (On a browser-only refresh with the server still running, `get_current_state()` returns the current `_selected_files` and the file picker merges them with git-changed files — see [File Picker — Auto-Selection](../5-webapp/file_picker.md#auto-selection).)
