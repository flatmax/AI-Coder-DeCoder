...
**Comment and string stripping:** Line comments (`%...`) are stripped at two levels: (1) a global copy of the source text has line comments removed for top-level pattern matching (`_FUNC_RE`, `_CLASS_RE`, `_VAR_RE`), and (2) each function's body text is independently stripped of both line comments and string literals (`'...'` and `"..."`) within `_extract_calls`, `_extract_local_vars`, and `_extract_read_vars` before scanning for identifiers. Only the per-function-body stripping removes strings — the global preprocessing strips only comments. This ensures that string contents inside function bodies don't produce false positives while keeping top-level pattern matching simple.
