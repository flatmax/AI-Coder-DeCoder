...
A `.docx` file auto-converted to markdown (via [Document Auto-Convert](#document-auto-convert)) and indexed:
...
**Note on CSV files:** CSV is already text and diffable. Auto-convert to a markdown table is most useful for small-to-medium files that benefit from being rendered as formatted tables in the doc index. Very large CSVs (thousands of rows) produce unwieldy markdown — users may prefer to exclude `.csv` from the `extensions` list in config and instead maintain a hand-written summary document.
...
The document auto-convert feature automatically converts non-markdown documents (`.docx`, `.pdf`, `.pptx`, `.xlsx`, `.csv`, `.rtf`, `.odt`) to markdown files, placing the converted output alongside the original. This replaces the previous manual-conversion recommendation with an automated pipeline that runs transparently during normal operation.

**Converted markdown is strictly superior in a git repo** — it's diffable, human-readable, greppable, and editable by the LLM via the standard edit block protocol. Auto-convert brings this benefit without requiring the user to run external tools manually.
...
If `markitdown` is not installed, auto-convert is disabled with a warning (same graceful degradation pattern as KeyBERT — see [Graceful Degradation in Packaged Releases](#graceful-degradation-in-packaged-releases)). Users running from source can install it with:
...
- Manifest records source hash, output path, conversion timestamp, image list
...
Server startup
    ...
    └── _start_background_doc_index() called AFTER "ready"
          ├── Phase 0: Auto-convert scan (if markitdown available)
          │     ├── Walk repo for convertible files (extension match, skip excluded dirs)
          │     ├── Hash check against .ac-dc/autoconvert_manifest.json
          │     ├── Convert new/changed files → sibling .md + extracted images
          │     ├── Show gitignore prompt toast if first detection
          │     └── Show action toasts for user-edit conflicts or naming conflicts
