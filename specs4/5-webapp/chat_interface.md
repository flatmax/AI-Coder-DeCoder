...
🎤 toggle for continuous voice dictation using the browser's Web Speech API.

**Component:** `SpeechToText` — a Lit component rendered inside the chat input area.

**Auto-transcribe mode:**
- ON: Start `SpeechRecognition` session. On utterance end, auto-restart after 100ms delay. Each utterance dispatches `transcript` CustomEvent. Continues until user clicks button again.
- OFF: Stops recognition.
- Configuration: `continuous: false` (auto-restart loop makes it feel continuous), `interimResults: false`, `lang: navigator.language`.
...
**Event flow:** Transcript event → insert at cursor position in textarea → add space separators → reposition cursor → auto-resize → run URL detection.

**Error handling:** Recognition error → stop, dispatch error event, revert to inactive. Component disconnect → `recognition.stop()`.

**Browser compatibility:** Web Speech API: Chromium-based browsers. Hide button if `SpeechRecognition` not available.
