...
`new_session` closes every live agent in addition to clearing main's history. The backend frees each agent's `ContextManager`, `StabilityTracker`, and file context, broadcasts `agentClosed {agent_id}` per agent, and then broadcasts `sessionChanged`. From the user's perspective, "new session" is a single gesture that dismisses the entire team along with main's conversation. Archives on disk survive — the dismissed agents remain browsable as read-only historical tabs via the history browser.
