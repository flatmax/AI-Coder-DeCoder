...
- The aggregate symbol-map or doc-map body in L0 excludes every path whose full content or compact block already appears elsewhere in the prompt
- The exclusion set is the union of selected files, user-excluded index files, and every path graduated into any cached tier as a file, symbol, or doc entry
- Three independent call sites compute this exclusion set: the prompt assembler, the cache-viewer's breakdown builder, and the cache-viewer's click-to-view modal dispatcher. All three must produce the same set, or the viewer's displayed token counts will diverge from the content actually sent to the LLM
- See [`specs-reference/3-llm/prompt-assembly.md § Symbol map exclusions`](../../specs-reference/3-llm/prompt-assembly.md#symbol-map-exclusions) for the detailed algorithm and call-site enumeration
...
Meta rows in cached tiers (L0 aggregate map) follow the exclusion-set contract above. Meta rows in the uncached tail (file tree, URLs, review, active files) reflect the rebuilt-every-request sections of the prompt. See [`specs-reference/3-llm/prompt-assembly.md § Synthetic meta rows`](../../specs-reference/3-llm/prompt-assembly.md#synthetic-meta-rows) for the complete key catalog and dispatch table.
