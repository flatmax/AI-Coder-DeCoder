...
class AiderChat:
    ...
    def request_changes(self, user_request, include_examples=True):
        """
        ...
        if include_examples:
            messages.extend(self.editor.get_example_messages())
        
        # File context
        file_context = self.editor.format_files_for_prompt()
        
        # User request with file context
        user_content = f"Here are the files:\n\n{file_context}\n\n{user_request}"
    ...
    def request_and_apply(self, user_request, dry_run=False, include_examples=True):
        ...
        """
        result = self.request_changes(user_request, include_examples)
