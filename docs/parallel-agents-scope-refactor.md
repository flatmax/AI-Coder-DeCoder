...
**Status:** complete. All 11 steps delivered. See IMPLEMENTATION_NOTES.md D24 for the summary entry and enumeration of candidate carve-outs / next steps.

Governing spec: `specs4/7-future/parallel-agents.md` § Foundation Requirements.
Decision log: IMPLEMENTATION_NOTES.md D22 (foundation uses the existing streaming pipeline); D24 (refactor completion and module-decomposition deferral).
...
- **Step 10** — delivered. New `TestConversationScopeDefault` test class added to `tests/test_llm_service.py` between `TestStreamingHappyPath` and `TestTurnIdPropagation`. Five tests pin the byte-identical contract: (1) `test_explicit_default_scope_produces_same_result` calls `_stream_chat` directly with `scope=service._default_scope()` — the production path future agent-spawning code will take; (2) `test_manually_constructed_scope_matches_default` builds a `ConversationScope` field-by-field (context, tracker, session_id, selected_files, archival_append closure over the history store) pointing at the same objects the helper would use, proving field semantics are the contract; (3) `test_none_fallback_produces_same_result` pins the safety-net path every method carries — `scope=None` falls through to the default, catching any future refactor that drops the fallback; (4) `test_default_scope_helper_fields` is a unit-level identity check — ContextManager, tracker, and selected_files must be the same objects (not copies) so mutations propagate, session_id matches by value; (5) `test_default_scope_without_history_store_has_none_append` pins the None-archival-append contract for tests that skip persistence. All three streaming tests bypass `chat_streaming` to invoke `_stream_chat` directly with explicit `_main_loop` / `_active_user_request` setup (matching what `chat_streaming` does under the hood) so the explicit-scope path is exercised independently. Observable surface matches `TestStreamingHappyPath`: persisted user + assistant messages, streamComplete event with correct request ID and response. The class provides the regression guard the refactor needs — any divergence between the implicit and explicit scope paths surfaces here before it reaches production.
- **Step 11** — delivered. IMPLEMENTATION_NOTES.md D24 records the refactor's completion, enumerates the 11-commit sequence with per-commit content summaries, pins the "main-conversation-only state stays on self" invariant for review mode, documents the `archival_append` closure as the agent-vs-main abstraction, defers `llm_service.py` decomposition with a sketch of candidate carve-outs for a future pass, and enumerates the concrete next steps for agent spawning (parser dispatch on AGENT blocks, `_spawn_agents_for_turn` orchestration, per-request accumulator routing for child streams, synthesis step). The refactor is complete — all per-conversation helpers in `_stream_chat`'s pipeline take `ConversationScope` explicitly, `TestConversationScopeDefault` pins the byte-identical contract, and the execution-layer prerequisite called out at the end of D22 is in place.
