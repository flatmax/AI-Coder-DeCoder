...
- **3.2b — synchronized pan/zoom.** `svg-pan-zoom` library integration. Both panels' viewports move together. Mouse wheel zoom centered on cursor. A floating fit button. `preserveAspectRatio="none"` override on the right panel so the future editor has viewBox-authority. The dependency needs adding to `package.json`.
